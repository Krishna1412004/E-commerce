...
// Source: ./app/(store)/product/[slug]/page.tsx
// Variable: allProductsQuery
// Query: *[_type == "product"]{        _id,        name,        "slug": slug.current    }
...
// Variable: query
// Query: *[_type == "product" && slug.current match $slug][0]{            _id,            name,            price,            currency,            stock,            description,            "slug": slug.current,            image {                asset->{                    _id,                    url                }            },            categories[]->        }
export type QueryResult = {
  _id: string;
  name: string | null;
  price: number | null;
  currency: "eur" | "gbp" | "inr" | "usd" | null;
  stock: number | null;
  description: Array<{
    children?: Array<{
      marks?: Array<string>;
...
// Query: *[_type == "product"] {    _id,    name,    price,    currency,    stock,    description,    "slug": slug.current,    image {      asset->{        _id,        url      }    },    "categories": categories[]->  } | order(name asc)
...
// Query: *[_type == "product" && slug.current == $slug][0]{    _id,    name,    price,    currency,    stock,    description,    slug,    image {        asset->{            _id,            url        }    },    categories[]->}
...
// Query: *[        _type == "product"        && (            name match $searchParam            || categories[]->name match $searchParam        )    ] {        _id,        name,        price,        currency,        stock,        description,        "slug": slug.current,        image {            asset->{                _id,                url            }        },        "categories": categories[]->    } | order(name asc)
